...
Menu::Menu() {
  ...
  if (!(menu_edit = load_png(("images/gui/menu_edit.png"), NULL))) {
    abort_on_error(
  ...
  if (!(levelSelectRight =
            load_png(("images/gui/levelSelectRight.png"), NULL))) {
    abort_on_error(
        "Cannot find image images/gui/levelSelectRight.png \n Please check "
        "your files and try again");
  }
  if (!(levelSelectNumber =
            load_png(("images/gui/levelSelectNumber.png"), NULL))) {
    abort_on_error(
  ...
  if (!(click = load_sample(("sounds/click.wav")))) {
    abort_on_error(
        "Cannot find sound sounds/click.png \n Please check your files and try "
        "again");
  }
  ...
  if (!(f1 = load_font(("fonts/arial_black.pcx"), NULL, NULL))) {
    abort_on_error(
        "Cannot find font fonts/arial_black.png \n Please check your files and "
        "try again");
  }
  ...
  if (!(music = load_sample("sounds/menu.ogg"))) {
    abort_on_error(
        "Cannot find music sounds/menu.ogg \n Please check your files and try "
        "again");
  }
...
void Menu::update() {
  ...
  if (!editMode) {
    ...
    // Hover edit
    else if (collisionAny(mouseX(), mouseX(), 60, 270, mouseY(), mouseY(), 700,
                          700 + 45)) {
      if (newSelectorY != 700) {
        newSelectorY = 700;
        selectorX = 60;
    ...
    // Hover help
    else if (collisionAny(mouseX(), mouseX(), 60, 270, mouseY(), mouseY(), 763,
                          763 + 45)) {
      if (newSelectorY != 763) {
        newSelectorY = 763;
        selectorX = 60;
    ...
    // Hover exit
    else if (collisionAny(mouseX(), mouseX(), 60, 270, mouseY(), mouseY(), 828,
                          828 + 45)) {
      if (newSelectorY != 828) {
        newSelectorY = 828;
        selectorX = 60;
    ...
    // Force start
    // if(joy[0].button[7].b)set_next_state(INGAME);
    //^^ Y U NO WORK?
    ...
    if (mouse_b & 1 || key[KEY_ENTER] || joy[0].button[0].b) {
      ...
      // level select right
      else if (collisionAny(mouseX(), mouseX(), 1200, 1240, mouseY(), mouseY(),
                            80, 120)) {
        play_sample(click, 255, 125, 1000, 0);
        while (mouse_b & 1) {
        }
