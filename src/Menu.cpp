...
Menu::Menu()
{
  ...
  if(!(music = FSOUND_Stream_Open("sounds/menu.mp3",2, 0, 0))){
      abort_on_error( "Cannot find music sounds/menu.mp3 \n Please check your files and try again");
  }
...
void Menu::update()
{
  ...
  if( !editMode){
    ...
    if(mouse_b & 1 || key[KEY_ENTER] || joy[0].button[0].b){
      ...
      // Start
      else if(collisionAny(mouseX(),mouseX(),60,270,mouseY(),mouseY(), 637, 637 + 45) || key[KEY_ENTER]){
