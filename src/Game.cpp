...
void Game::init() {
  ...
  for (unsigned int i = 0; i < tile_map->mapTiles.size(); i++) {
    if (tile_map->mapTiles.at(i).getType() == tile_spawn_mutant) {
      enemy* newBadGuy =
          new enemy(tile_map->mapTiles.at(i).getX(),
                    tile_map->mapTiles.at(i).getY(), enemy_mutant);
...
void Game::update() {
  ...
  if (frames_done % 2 == 0) {
    ...
    for (unsigned int i = 0; i < badGuy.size(); i++) {
  ...
  if (key[KEY_R]) {
    enemy* newBadGuy =
        new enemy(character.getX(), character.getY(), enemy_mutant);
    newBadGuy->load_images();
    newBadGuy->load_sounds();
    badGuy.push_back(newBadGuy);
    rest(10);
  ...
  if (key[KEY_T]) {
    enemy* newBadGuy =
        new enemy(character.getX(), character.getY(), enemy_sentry);
    newBadGuy->load_images();
    newBadGuy->load_sounds();
    badGuy.push_back(newBadGuy);
    rest(100);
