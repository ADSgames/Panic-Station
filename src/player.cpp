...
player::player() {
  ...
  weapons[0] =
      new weapon(100, 0, 0, 0, 0, 0, 0, "images/HUD/pistol_small.png", "Arm");
  weapons[1] = new weapon(0, 60, 8, 0, 50, -1, 10,
                          "images/HUD/pistol_small.png", "Pistol");
  weapons[2] =
      new weapon(0, 60, 30, 0, 100, 3, 10, "images/HUD/mp5_small.png", "MP5");
  weapons[3] =
      new weapon(0, 60, 71, 0, 200, 2, 10, "images/HUD/ppsh_small.png", "PPSH");
  weapons[4] = new weapon(0, 60, 500, 0, 1000, 0, 10,
                          "images/HUD/minigun_small.png", "Minigun");
  weapons[5] = new weapon(0, 60, 5, 0, 200, 25, 10,
                          "images/HUD/remington_small.png", "Remington");
...
void player::load_images() {
  player_images[0] = load_png("images/character/walk_1.png", NULL);
  player_images[1] = load_png("images/character/walk_2.png", NULL);
  player_images[2] = load_png("images/character/walk_3.png", NULL);
  player_images[3] = load_png("images/character/walk_4.png", NULL);

  player_images[4] = load_png("images/character/up_1.png", NULL);
  player_images[5] = load_png("images/character/up_2.png", NULL);
  player_images[6] = load_png("images/character/up_3.png", NULL);
  player_images[7] = load_png("images/character/up_4.png", NULL);

  player_images[8] = load_png("images/character/jump_1.png", NULL);
  player_images[9] = load_png("images/character/jump_2.png", NULL);
  player_images[10] = load_png("images/character/jump_3.png", NULL);
  player_images[11] = load_png("images/character/jump_4.png", NULL);
  player_images[12] = load_png("images/character/jump_5.png", NULL);
  player_images[13] = load_png("images/character/jump_6.png", NULL);

  player_images[15] = load_png("images/character/crouch.png", NULL);
  player_images[16] = load_png("images/character/crouch_shoot.png", NULL);

  player_images[17] = load_png("images/character/idle_1.png", NULL);
  ...
  statusbar = load_png("images/HUD/statusbar.png", NULL);
  gui_paper = load_png("images/HUD/gui_paper.png", NULL);
  gui_statue = load_png("images/HUD/gui_statue.png", NULL);

  gui_pistol = load_png("images/HUD/pistol.png", NULL);
  gui_mp5 = load_png("images/HUD/mp5.png", NULL);
  gui_ppsh = load_png("images/HUD/ppsh.png", NULL);
  gui_minigun = load_png("images/HUD/minigun.png", NULL);
  gui_remington = load_png("images/HUD/remington.png", NULL);
}
...
void player::draw(BITMAP* temp, int tile_map_x, int tile_map_y) {
  ...
  if (!jumping_animation_done) {
    if (characterDir == LEFT) {
      draw_sprite(temp, player_images[8 + jumping_animation_sequence / 2],
                  x - tile_map_x, y - tile_map_y);
    } else {
      draw_sprite_h_flip(temp,
                         player_images[8 + jumping_animation_sequence / 2],
                         x - tile_map_x, y - tile_map_y);
    }
  }
  ...
  // Falling
    ...
    if (characterDir == LEFT) {
      ...
      draw_sprite_h_flip(temp, player_images[13], x - tile_map_x,
                         y - tile_map_y);
    }
  }
  ...
  // Walking
    ...
    if (height == 0) {
      if (idle) {
        if (characterDir == LEFT) {
          ...
          draw_sprite_h_flip(temp, player_images[17], x - tile_map_x,
                             y - tile_map_y);
        }
      } else if (player_images[characterDir + walking_animation_sequence /
                                                  ANIMATION_SPEED] != NULL) {
        if (characterDir == LEFT) {
          draw_sprite(
              temp, player_images[walking_animation_sequence / ANIMATION_SPEED],
              x - tile_map_x, y - tile_map_y);
        } else {
          draw_sprite_h_flip(
              temp, player_images[walking_animation_sequence / ANIMATION_SPEED],
    ...
    // Crouching
      ...
      draw_sprite_h_flip(temp, player_images[15], x - tile_map_x,
                         y - tile_map_y);
    }
  }

  textprintf_ex(temp, pixelart_2, 400, 50, makecol(255, 255, 255),
                makecol(0, 0, 0), "Dir:%i  Ani:%i  Full:%i", characterDir,
  ...
  if (characterDir == RIGHT)
    hand_offsetX += 6;
  if ((walking_animation_sequence / ANIMATION_SPEED == 1 ||
       walking_animation_sequence / ANIMATION_SPEED == 3) &&
      !idle)
  ...
  // Draw sweps
  if (characterDir == LEFT) {
    weapons[currentWeapon]->draw(temp, x - tile_map_x + hand_offsetX,
                                 y - tile_map_y + hand_offsetY, false);
  } else if (characterDir == RIGHT) {
    weapons[currentWeapon]->draw(temp, x - tile_map_x + hand_offsetX,
                                 y - tile_map_y + hand_offsetY, true);
  }

  // Draw HUD
  if (!isReadingBlock) {
    textprintf_ex(temp, pixelart_2, 224, 10, makecol(255, 255, 255),
                  makecol(0, 0, 0), "%i", stamina);
    textprintf_ex(temp, pixelart_2, 224, 50, makecol(255, 255, 255),
                  makecol(0, 0, 0), "%i", health);
    if (headInLiquid)
      textprintf_ex(temp, pixelart_2, 224, 85, makecol(255, 255, 255),
                    makecol(0, 0, 0), "%i", oxygen);
    ...
    textprintf_ex(temp, pixelart, 5, 180, makecol(255, 255, 255),
                  makecol(0, 0, 0), "Deathcount:%i", deathcount);
    textprintf_ex(temp, pixelart, 5, 200, makecol(255, 255, 255),
                  makecol(0, 0, 0), "Weapon:%i", currentWeapon);

    if (isTalkingToNpc) {
      if (npc_ID == 0)
        textprintf_ex(temp, munro_large, 20, SCREEN_H - 62,
                      makecol(255, 255, 255), makecol(0, 0, 0),
                      "Hi! Head on inside, Gerald is waiting. Good luck!");
      if (npc_ID == 1) {
        textprintf_ex(temp, munro_large, 20, SCREEN_H - 108,
                      makecol(255, 255, 255), makecol(0, 0, 0),
                      "Hey man, I've been hearing weird noises from the "
                      "basement, you should");
        textprintf_ex(temp, munro_large, 20, SCREEN_H - 62,
                      makecol(255, 255, 255), makecol(0, 0, 0),
                      "go check it out!");
    ...
    if (headInLiquid)
      draw_sprite(temp, statusbar, 10, 80);
    if (currentWeapon == 1)
      draw_sprite(temp, gui_pistol, 280, 10);
    if (currentWeapon == 2)
      draw_sprite(temp, gui_mp5, 280, 10);
    if (currentWeapon == 3)
      draw_sprite(temp, gui_ppsh, 280, 10);
    if (currentWeapon == 4)
      draw_sprite(temp, gui_minigun, 280, 10);
    if (currentWeapon == 5)
      draw_sprite(temp, gui_remington, 280, 10);

    rectfill(temp, 14, 14, 14 + (stamina * 2), 33,
             (255, 255, (stamina * 2) + 55));
    rectfill(temp, 14, 49, 14 + (health * 2), 68,
             makecol((health * 2) + 55, 0, 0));

    if (headInLiquid)
      rectfill(
          temp, 14, 84, 14 + (oxygen * 2), 103,
          makecol((oxygen * 2) + 55, (oxygen * 2) + 55, (oxygen * 2) + 55));
  }

  if (isReadingBlock) {
    if (read_ID == 0) {
      draw_sprite(temp, gui_paper, 0, 0);
      textprintf_ex(temp, munro_large, 200, 100, makecol(0, 0, 0),
                    makecol(0, 0, -1), "Look! This page has words!");
    }
    if (read_ID == 1) {
      draw_sprite(temp, gui_statue, 0, 0);
      textprintf_ex(temp, munro_large, 400, 600, makecol(0, 0, 0),
                    makecol(0, 0, -1), "The statue has been liberated!");
    }
  }
...
void player::update(tileMap* newMap) {
  ...
  for (unsigned int i = 0; i < newMap->mapTiles.size(); i++) {
    if (collisionAny(
      ...
      if (newMap->mapTiles.at(i).getAttribute() == solid) {
        if (collisionAny(x + 4, x + 68, newMap->mapTiles.at(i).getX(),
                         newMap->mapTiles.at(i).getX() +
                             newMap->mapTiles.at(i).getWidth(),
                         y + height + 16, y + 128,
                         newMap->mapTiles.at(i).getY(),
      ...
      if (newMap->mapTiles.at(i).getAttribute() == solid) {
        if (collisionAny(x + 16, x + 48, newMap->mapTiles.at(i).getX(),
                         newMap->mapTiles.at(i).getX() +
                             newMap->mapTiles.at(i).getWidth(),
                         y + 16, y + 144, newMap->mapTiles.at(i).getY(),
                         newMap->mapTiles.at(i).getY() +
                             newMap->mapTiles.at(i).getHeight())) {
          canClimbDown = false;
        }
      ...
      if (newMap->mapTiles.at(i).getAttribute() != gas &&
          ...
          newMap->mapTiles.at(i).getAttribute() != interactive &&
      ...
      if (newMap->mapTiles.at(i).getAttribute() != gas &&
          newMap->mapTiles.at(i).getAttribute() != liquid &&
          newMap->mapTiles.at(i).getAttribute() != climb &&
          newMap->mapTiles.at(i).getAttribute() != interactive) {
        if (collisionAny(x + 16, x + 48, newMap->mapTiles.at(i).getX(),
                         newMap->mapTiles.at(i).getX() +
                             newMap->mapTiles.at(i).getWidth(),
            ...
            collisionTop(newMap->mapTiles.at(i).getY(),
                         ...
                         y + yVelocity, y + 144 + yVelocity + 16)) {
          canJumpUp = false;
        }
      }
      ...
      if (newMap->mapTiles.at(i).getAttribute() == liquid) {
        if (collisionAny(x + 16, x + 48, newMap->mapTiles.at(i).getX(),
                         newMap->mapTiles.at(i).getX() +
                             newMap->mapTiles.at(i).getWidth(),
                         y, y + 20, newMap->mapTiles.at(i).getY(),
                         newMap->mapTiles.at(i).getY() +
                             newMap->mapTiles.at(i).getHeight())) {
          headInLiquid = true;
        }
      ...
      if (newMap->mapTiles.at(i).getAttribute() == cover) {
        if (collisionAny(x + 4, x + 8, newMap->mapTiles.at(i).getX(),
                         newMap->mapTiles.at(i).getX() +
                             newMap->mapTiles.at(i).getWidth(),
                         y + height + 16, y + 128,
                         newMap->mapTiles.at(i).getY(),
                         newMap->mapTiles.at(i).getY() +
                             newMap->mapTiles.at(i).getHeight()) &&
            collisionRight(x + 4, x + 8, newMap->mapTiles.at(i).getX(),
                           newMap->mapTiles.at(i).getX() +
                               newMap->mapTiles.at(i).getWidth())) {
      ...
      if (newMap->mapTiles.at(i).getAttribute() == item) {
        if (collisionAny(x + 18, x + 46, newMap->mapTiles.at(i).getX(),
                         newMap->mapTiles.at(i).getX() +
                             newMap->mapTiles.at(i).getWidth(),
                         y + 16 + height, y + 128,
                         newMap->mapTiles.at(i).getY(),
          ...
          if (newMap->mapTiles.at(i).getType() == tile_pistol)
            weaponAquired = 1;
          if (newMap->mapTiles.at(i).getType() == tile_mp5)
            weaponAquired = 2;
          if (newMap->mapTiles.at(i).getType() == tile_ppsh)
            weaponAquired = 3;
          if (newMap->mapTiles.at(i).getType() == tile_minigun)
            weaponAquired = 4;
          if (newMap->mapTiles.at(i).getType() == tile_remington)
            weaponAquired = 5;
          if (weaponAquired != 0) {
            play_sample(weapon_pickup, 255, 125, 1000, 0);

            /*if(weapons[weaponAquired].getMagazine() == 0)
             weapons[weaponAquired].setMagazine(
            weapons[weaponAquired].getMagazine() +
            weapons[weaponAquired].getMagazineSize()); else
             weapons[weaponAquired].setAmmunition(
            weapons[weaponAquired].getAmmunition() +
            weapons[weaponAquired].getMagazineSize());*/
          ...
          if (newMap->mapTiles.at(i).getType() == tile_pistol_magazine)
            ammunitionAquired = 1;
          if (newMap->mapTiles.at(i).getType() == tile_ammo_mp5)
            ammunitionAquired = 2;
          if (newMap->mapTiles.at(i).getType() == tile_ammo_ppsh)
            ammunitionAquired = 3;
          if (newMap->mapTiles.at(i).getType() == tile_ammo_minigun)
            ammunitionAquired = 4;
          if (newMap->mapTiles.at(i).getType() == tile_ammo_remington)
            ammunitionAquired = 5;

          if (ammunitionAquired != 0) {
            weapons[ammunitionAquired]->addAmmo(
                weapons[ammunitionAquired]->getMagazineSize());
    ...
    if (key[KEY_E] || joy[0].button[2].b) {
      // particle newParticle( x + random( 0, 40), y + random( 0, 40), color,
      // -2, 2, -2, 2, CIRCLE, 1);
      if (collisionAny(
              x + 18, x + 46, newMap->mapTiles.at(i).getX(),
              newMap->mapTiles.at(i).getX() + newMap->mapTiles.at(i).getWidth(),
    ...
    if (key[KEY_E] || joy[0].button[2].b) {
      if (collisionAny(
        ...
        if (newMap->mapTiles.at(i).getAttribute() == interactive) {
          // Flicking switch
          if (newMap->mapTiles.at(i).getType() == tile_generator_off &&
              switch_loop > 20) {
            newMap->mapTiles.at(i).setType(44);
            switch_loop = 0;
            play_sample(Switch, 255, 125, 1000, 0);
            switch_direction = true;
            switch_frame = 0;
          }
          if (newMap->mapTiles.at(i).getType() == tile_generator_on &&
              switch_loop > 20) {
            newMap->mapTiles.at(i).setType(45);
            switch_loop = 0;
            play_sample(Switch, 255, 125, 1000, 0);
            switch_direction = false;
            switch_frame = 0;
          ...
          if (newMap->mapTiles.at(i).getType() == tile_light_cage_1 &&
              switch_loop > 10) {
            newMap->mapTiles.at(i).setType(47);
            switch_loop = 0;
            play_sample(glass, 255, 125, random(500, 1500), 0);
            switch_frame = 0;
          }
          if (newMap->mapTiles.at(i).getType() == tile_light_cage_2 &&
              switch_loop > 10) {
            newMap->mapTiles.at(i).setType(48);
            switch_loop = 0;
            play_sample(glass, 255, 125, random(500, 1500), 0);
            switch_frame = 0;
          }
          ...
          // Use elevator
          if (newMap->mapTiles.at(i).getType() == tile_elevator_1 &&
              switch_loop > 20) {
            newMap->mapTiles.at(i).setType(51);
            switch_loop = 0;
            play_sample(elevator, 255, 125, 1000, 0);
          ...
          if (newMap->mapTiles.at(i).getType() == tile_crate1_1 &&
              switch_loop > 10) {
            newMap->mapTiles.at(i).setType(84);
            switch_loop = 0;
            play_sample(wood_smash, 255, 125, random(500, 1500), 0);
            switch_frame = 0;
          }
          if (newMap->mapTiles.at(i).getType() == tile_crate1_2 &&
              switch_loop > 10) {
            newMap->mapTiles.at(i).setType(85);
            switch_loop = 0;
            play_sample(wood_smash, 255, 125, random(500, 1500), 0);
            switch_frame = 0;
          }
          if (newMap->mapTiles.at(i).getType() == tile_crate1_3 &&
              switch_loop > 10) {
            newMap->mapTiles.at(i).setType(86);
            switch_loop = 0;
            play_sample(wood_smash, 255, 125, random(500, 1500), 0);
            switch_frame = 0;
          }
          ...
          // Door
          if (newMap->mapTiles.at(i).getType() == tile_brick_door_1 &&
              switch_loop > 10) {
            newMap->mapTiles.at(i).setType(108);
            switch_loop = 0;
            play_sample(squeak_door, 255, 125, 1000, 0);
            switch_frame = 0;
          }
          if (newMap->mapTiles.at(i).getType() == tile_brick_door_2 &&
              switch_loop > 10) {
            newMap->mapTiles.at(i).setType(107);
            switch_loop = 0;
            play_sample(squeak_door, 255, 125, 1000, 0);
            switch_frame = 0;
          }
          if (newMap->mapTiles.at(i).getType() == tile_brown_door_1 &&
              switch_loop > 10) {
            newMap->mapTiles.at(i).setType(118);
            switch_loop = 0;
            play_sample(squeak_door, 255, 125, 1000, 0);
            switch_frame = 0;
          }
          if (newMap->mapTiles.at(i).getType() == tile_brown_door_2 &&
              switch_loop > 10) {
            newMap->mapTiles.at(i).setType(117);
            switch_loop = 0;
            play_sample(squeak_door, 255, 125, 1000, 0);
            switch_frame = 0;
          }
          if (newMap->mapTiles.at(i).getType() == tile_door_1 &&
              switch_loop > 10) {
            newMap->mapTiles.at(i).setType(120);
            switch_loop = 0;
            play_sample(squeak_door, 255, 125, 1000, 0);
            switch_frame = 0;
          }
          if (newMap->mapTiles.at(i).getType() == tile_door_2 &&
              switch_loop > 10) {
            newMap->mapTiles.at(i).setType(119);
            switch_loop = 0;
            play_sample(squeak_door, 255, 125, 1000, 0);
            switch_frame = 0;
          }
          // Mailbox
          if (newMap->mapTiles.at(i).getType() == tile_mailbox_1 &&
              switch_loop > 10) {
            newMap->mapTiles.at(i).setType(6);
            switch_loop = 0;
            play_sample(squeak_mailbox, 255, 125, 1000, 0);
            switch_frame = 0;
          }
          if (newMap->mapTiles.at(i).getType() == tile_mailbox_2 &&
              switch_loop > 10) {
            newMap->mapTiles.at(i).setType(5);
            switch_loop = 0;
            play_sample(squeak_mailbox, 255, 125, 1000, 0);
            switch_frame = 0;
          }
    ...
    if (key[KEY_Q] || joy[0].button[3].b) {
      if (collisionAny(
              ...
              newMap->mapTiles.at(i).getY() +
                  newMap->mapTiles.at(i).getHeight())) {
        if (newMap->mapTiles.at(i).getAttribute() == interactive) {
          if (newMap->mapTiles.at(i).getType() == tile_elevator_33 ||
              newMap->mapTiles.at(i).getType() == tile_brick_door_2 ||
              newMap->mapTiles.at(i).getType() == tile_brown_door_2) {
            finishLevel = true;
            if (level == 1)
              newMap->load("data/levels/level_02");
            if (level == 2)
              newMap->load("data/levels/level_03");
            if (level == 3)
              newMap->load("data/levels/level_04");
            if (level == 4)
              newMap->load("data/levels/level_05");
            if (level == 5)
              newMap->load("data/levels/level_06");
            if (level == 6)
              newMap->load("data/levels/level_07");
            if (level == 7)
              newMap->load("data/levels/level_08");
            if (level == 8)
              newMap->load("data/levels/level_09");
            if (level == 9)
              newMap->load("data/levels/level_10");
            if (level == 10)
              newMap->load("data/levels/level_11");
            level++;
    ...
    if (switch_direction) {
      if (switch_frame > 3)
        if (newMap->mapTiles.at(i).getType() == tile_generator_2) {
          newMap->mapTiles.at(i).setType(43);
          switch_frame = 0;
        }
      if (switch_frame > 3)
        if (newMap->mapTiles.at(i).getType() == tile_generator_1) {
          newMap->mapTiles.at(i).setType(45);
          switch_frame = 0;
        }
    }
    if (!switch_direction) {
      ...
      if (switch_frame > 3)
        if (newMap->mapTiles.at(i).getType() == tile_generator_1) {
          newMap->mapTiles.at(i).setType(25);
          switch_frame = 0;
        }
    }
    ...
    if (elevator_direction) {
      ...
      if (switch_frame > 3) {
        ...
        if (newMap->mapTiles.at(i).getType() >= tile_elevator_3 &&
            newMap->mapTiles.at(i).getType() <= tile_elevator_32) {
    ...
    if (newMap->mapTiles.at(i).getAttribute() == solid) {
      if (collisionAny(
              ...
              newMap->mapTiles.at(i).getY() +
                  newMap->mapTiles.at(i).getHeight())) {
        if (newMap->mapTiles.at(i).getType() == tile_cement_1)
          material = 1;
        if (newMap->mapTiles.at(i).getType() == tile_cement_2)
          material = 1;
        if (newMap->mapTiles.at(i).getType() == tile_cement_3)
          material = 1;
        if (newMap->mapTiles.at(i).getType() == tile_cement_4)
          material = 1;
        if (newMap->mapTiles.at(i).getType() == tile_cement_5)
          material = 1;
        if (newMap->mapTiles.at(i).getType() == tile_cement_6)
          material = 1;
        if (newMap->mapTiles.at(i).getType() == tile_cement_7)
          material = 1;
        if (newMap->mapTiles.at(i).getType() == tile_cement_8)
          material = 1;
      }
    }
  }
  ...
  for (unsigned int i = 0; i < newMap->mapTiles.size(); i++) {
    ...
    if (newMap->mapTiles.at(i).getAttribute() == harmful) {
      if (collisionAny(
              x + 18, x + 46, newMap->mapTiles.at(i).getX(),
              newMap->mapTiles.at(i).getX() + newMap->mapTiles.at(i).getWidth(),
              y - 16 + height, y + 128, newMap->mapTiles.at(i).getY(),
              newMap->mapTiles.at(i).getY() +
                  newMap->mapTiles.at(i).getHeight() + 64)) {
        if (newMap->mapTiles.at(i).getType() == 97)
          play_sample(death_radioactive, 122, 125, 1000, 0);
        else if (newMap->mapTiles.at(i).getType() == 98)
          play_sample(death_radioactive, 122, 125, 1000, 0);
        else if (newMap->mapTiles.at(i).getType() == 99)
          play_sample(death_radioactive, 122, 125, 1000, 0);
        else if (newMap->mapTiles.at(i).getType() == 100)
          play_sample(death_radioactive, 122, 125, 1000, 0);
        dead = true;
      }
    }
  ...
  if (key[KEY_A] || joy[0].stick[0].axis[0].d1 || key[KEY_LEFT]) {
    if ((!key[KEY_D] || joy[0].stick[0].axis[0].d2) && jumping_animation_done &&
        x > 0) {
      if (inLiquid)
        play_sample(water_walk, 255, 125, 1000, 0);
      ...
      if (newMap->mapTiles.at(i).getAttribute() == solid ||
        ...
        if (collisionAny(x + 16, x + 48, newMap->mapTiles.at(i).getX(),
          ...
          if (!collisionAny(x + 16, x + 48, newMap->mapTiles.at(i).getX(),
                            newMap->mapTiles.at(i).getX() +
                                newMap->mapTiles.at(i).getWidth(),
                            y + 16, y + 129, newMap->mapTiles.at(i).getY(),
                            newMap->mapTiles.at(i).getY() +
                                newMap->mapTiles.at(i).getHeight()) &&
              !collisionTop(y + 16, y + 129, newMap->mapTiles.at(i).getY(),
                            newMap->mapTiles.at(i).getY() +
                                newMap->mapTiles.at(i).getHeight() + 65)) {
            smoothFall = true;
          }
        }
      }
  ...
  // Shoot currentWeapon
  if ((joy[0].stick[0].axis[2].d1 || key[KEY_ENTER]) && !shooting &&
      currentWeapon != 0) {
    ...
    if (canShoot) {
      if (weapons[currentWeapon]->getMagazine() > 0 && canShoot) {
        ...
        if (weapons[currentWeapon]->getBulletsFired() == 1) {
          ...
          if (height == 0) {
            for (int i = 0; i < weapons[currentWeapon]->getBulletsFired();
                 i++) {
              projectile newBullet(0, x + 30, y + bulletHeight - (i * 10),
                                   bulletSpeed * bulletDirection);
              bullets.push_back(newBullet);
            }
          }
        }
        weapons[currentWeapon]->setMagazine(
            weapons[currentWeapon]->getMagazine() - 1);
      } else
