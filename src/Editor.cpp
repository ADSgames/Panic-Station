...
void Editor::update()
{
  ...
  if( saving){
    ...
    if( mouse_b & 1){
      ...
      if( layer == 1){
        for(int i = 0; i < tile_map -> mapTiles.size(); i++){
          if(collisionAny(mouse_x + tile_map -> x, mouse_x + tile_map -> x, tile_map -> mapTiles.at(i).getX(), tile_map -> mapTiles.at(i).getX() + tile_map -> mapTiles.at(i).getWidth(), mouse_y + tile_map -> y, mouse_y + tile_map -> y, tile_map -> mapTiles.at(i).getY(), tile_map -> mapTiles.at(i).getY() + tile_map -> mapTiles.at(i).getHeight())){
...
void Editor::draw( bool toScreen)
{
  ...
  if( saving){
    ...
    // Cursor
    circlefill(buffer, mouse_x * resDiv, mouse_y * resDiv, 10, makecol(0,0,0));
    circlefill(buffer, mouse_x * resDiv, mouse_y * resDiv, 8, makecol(255,255,255));
  }
