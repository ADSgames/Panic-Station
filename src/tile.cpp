...
void tile::setType(int newType) {
  ...
  if (tileSprites[type][1] != NULL) {
    setImagesAnimated(tileSprites[type][0], tileSprites[type][1],
                      tileSprites[type][2], tileSprites[type][3]);
  } else {
...
// Set tiles images
void tile::setImagesAnimated(BITMAP* image1,
                             BITMAP* image2,
                             BITMAP* image3,
                             BITMAP* image4) {
  images[0] = image1;
...
void tile::draw_tile(BITMAP* tempSprite, int xOffset, int yOffset, int frame) {
  ...
  // Image not available? draw debug
    ...
    textprintf_ex(tempSprite, font, x - xOffset, y - yOffset, makecol(0, 0, 0),
                  -1, "Image");
    textprintf_ex(tempSprite, font, x - xOffset, y - yOffset + 20,
